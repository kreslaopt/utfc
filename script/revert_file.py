...
def revert_file(file_path):
    """Возвращает пути в поле 'images' к исходному виду: добавляет ../ в начало."""
    try:
            ...
            for image_set in data["images"]:
                for key, value in image_set.items():
                    if isinstance(value, str) and not value.startswith("../"):
                        image_set[key] = f"../{value}"

        with open(file_path, 'w', encoding='utf-8') as file:
            json.dump(data, file, ensure_ascii=False, indent=4)

        print(f"Файл {file_path} успешно обработан (возврат к исходному виду).")

    except Exception as e:
        print(f"Ошибка при обработке файла {file_path}: {e}")

def find_and_revert_files(root_dir):
...
if __name__ == "__main__":
    ...
    find_and_revert_files(root_directory)
    print("Возврат к исходному виду завершён.")
